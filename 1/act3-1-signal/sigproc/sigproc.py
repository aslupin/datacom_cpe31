...
class Signal(object):
    ...
    def read_time_function(self,func,clear=True,offset=0,duration=None):
        ...
        if clear:
            ts.loc[interval,"a"] = func(ts[interval].t.values-offset)
        else:
            ts.loc[interval,"a"] += func(ts[interval].t.values-offset)
    ...
    def concat(*signals):
        ...
        """
        if len(signals) == 0:
            raise Exception("At least one signal must be provided")
        duration = sum([s.duration for s in signals])
        newsig = Signal(sampling_rate=signals[0].sampling_rate,duration=duration)
        offset = 0
        for s in signals:
            newsig.mix(s,offset=offset)
            offset += s.duration
        return newsig
    ...
    def plot_time_and_freq(self,min_amplitude=1e-4,
            tfig_options={},ffig_options={},tline_options={},fline_options={}):
        if not NOTEBOOK_SESSION:
            raise Exception("This method is available in a notebook session only.")
        tfig_cnf = {
                **DEFAULT_PLOT_SETTINGS["time_fig_options"], 
                **DEFAULT_PLOT_SETTINGS["time_freq_fig_options"], 
                **tfig_options}
        tline_cnf = {**DEFAULT_PLOT_SETTINGS["time_line_options"], **tline_options}
        ffig_cnf = {
                **DEFAULT_PLOT_SETTINGS["freq_fig_options"], 
                **DEFAULT_PLOT_SETTINGS["time_freq_fig_options"], 
                **ffig_options}
        fline_cnf = {**DEFAULT_PLOT_SETTINGS["freq_line_options"], **fline_options}

        ts = self.time_domain
        tfig = bkp.figure(**tfig_cnf)
        tfig.line(ts.t*1000,ts.a,**tline_cnf)

        fs = self.freq_domain
        fs = fs[fs.a >= min_amplitude]
        ffig = bkp.figure(**ffig_cnf)
        ffig.segment(fs.f,0,fs.f,fs.a,**fline_cnf)

        grid = bkp.gridplot([[tfig,ffig]])
        bkp.show(grid)
