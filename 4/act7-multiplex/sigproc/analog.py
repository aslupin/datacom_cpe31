"""
QAM Module - Simple QAM signal manipulation
...
class Qam:
    ...
    #################################
    def __init__(self,modulation,bitrate,fc,format='mp'):
        '''
        Create a modulator instance using the symbol mapping provided via the
        parameter modulation, whose entries are of the form:

          <data-bit(s)> : (<magnitude>,<phase-degrees>)

        However, if the format option is set to 'iq', each entry is expected
        to be of the form:

          <data-bit(s)> : (<i-amplitude>,<q-amplitude>)
        ...
        if format.lower() == 'iq':
            ...
            for key in modulation:
                i,q = modulation[key]
                z = i+q*1j
                self.modulation[key] = (abs(z),np.degrees(np.angle(z)))
        else:
            self.modulation = deepcopy(modulation)
    ...
    #################################
    @staticmethod
    def generate_frequency(hz,duration,phase=0,magnitude=1):
        s = Signal(duration=duration)
        s.read_time_function(lambda t: magnitude*np.sin(2*np.pi*hz*t + np.radians(phase)))
        return s
    ...
    def constellation(self):
        return [(a*np.cos(np.radians(p)),a*np.sin(np.radians(p)),bits) 
